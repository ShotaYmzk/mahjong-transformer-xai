...
`scripts/train_transformer_v2_stream_xml.py`

巨大コーパスを HDF5 に落とさず、XML をバッファしながら学習する代替入口（既定ハイパーは HDF5 本番より小さいので揃えること）。

`scripts/train_transformer_v2.py`

単一 NPZ を `TensorDataset` に載せて短時間で回すデバッグ用学習入口（サンプル形状から `MahjongTransformerConfig` の `static_dim` 等を推定）。
